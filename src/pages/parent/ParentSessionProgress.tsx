...
const ParentSessionProgress: React.FC = () => {
  ...
  }, [selectedStudent, filter, currentPage]);
            ...
            {sessionData?.stats.subjectBreakdown && sessionData.stats.subjectBreakdown.length > 0 ? (
              <div className="flex flex-wrap gap-3">
                {sessionData.stats.subjectBreakdown.map((subject, index) => (
                  <Badge key={index} className="bg-blue-400/20 text-blue-300 border-blue-400/30 px-4 py-2">
                    ...
                    <div className="flex items-center gap-1">
                      {[...Array(5)].map((_, i) => (
                        <StarIcon
                          key={i}
                          className={`h-5 w-5 ${
                            i < selectedSession.studentRating!
                              ? 'text-yellow-300 fill-yellow-300'
                              : 'text-white/20'
                          }`}
                        />
