...
const ManageTutorApplicationsPage: React.FC = () => {
  ...
  // Session types the admin selects to approve for the tutor
  const [selectedSessionTypes, setSelectedSessionTypes] = useState<('one-on-one' | 'group' | 'consultation')[]>(['one-on-one', 'group', 'consultation']);
  ...
  const handleApproveApplication = (application: TutorApplication) => {
    ...
    setAdminNotes("");
    // Pre-fill with tutor's preferred session types, or all types if not specified
    setSelectedSessionTypes(application.preferred_session_types || ['one-on-one', 'group', 'consultation']);
  ...
  const handleActionSubmit = async () => {
    ...
    try {
      ...
      if (actionType === "approve") {
        console.log('✅ ADMIN APPROVING TUTOR APPLICATION - User ID:', selectedApplication.user_id, 'Application ID:', selectedApplication.id, 'Session Types:', selectedSessionTypes);
            ...
            <div className="mt-3">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <span
                        className={`ml-2 ${selectedApplication.specializes_learning_disabilities
                          ? "text-green-600"
                          : "text-gray-500"
                          }`}
                      >
                ...
                <div>
                  ...
                  <div className="bg-muted p-4 rounded-lg">
                    {selectedApplication.preferred_session_types && selectedApplication.preferred_session_types.length > 0 ? (
                      <div className="flex flex-wrap gap-2">
                        {selectedApplication.preferred_session_types.map((type) => (
                          <span
                            key={type}
                      ...
                      <p className="text-sm text-muted-foreground">No session types specified</p>
                    )}
                  </div>
                </div>
          ...
          <div className="relative top-8 mx-auto p-5 border w-11/12 md:w-1/2 shadow-lg rounded-md bg-card border-border max-h-[85vh] overflow-y-auto">
            <div className="mt-3">
              ...
              <div className="space-y-4">
                ...
                {/* Session Types Selection for Approval */}
                {actionType === "approve" && (
                  <div>
                    <label className="block text-sm font-medium text-card-foreground mb-2">
                      Approve for Session Types *
                    </label>
                    <p className="text-xs text-muted-foreground mb-3">
                      {selectedApplication?.preferred_session_types?.length ? (
                        <>Tutor requested: <strong>{selectedApplication.preferred_session_types.join(', ')}</strong></>
                      ) : (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                      {[
                        ...
                        <label
                          key={type.id}
                          className={`flex items-center p-3 rounded-lg border-2 cursor-pointer transition-all ${selectedSessionTypes.includes(type.id)
                        ...
                        >
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                setSelectedSessionTypes([...selectedSessionTypes, type.id]);
                              } else {
                                setSelectedSessionTypes(selectedSessionTypes.filter((t) => t !== type.id));
                              }
                            }}
                            className="sr-only"
                          />
                    ...
                    {selectedSessionTypes.length === 0 && (
                      <p className="text-xs text-red-500 mt-2">Please select at least one session type</p>
                    )}
                  </div>
                )}
                ...
                <div className="flex justify-end space-x-3 pt-4 border-t border-border">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-medium text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all duration-200 shadow-[0_2px_2px_0_#16803D] hover:shadow-[0_4px_4px_0_#16803D] transform hover:scale-105 ${actionType === "approve"
                    ...
                    disabled={
                      ...
                      (actionType === "approve" && selectedSessionTypes.length === 0)
                    }
                  >
                    {processingAction ? (
                      <>
